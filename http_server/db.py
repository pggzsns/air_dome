...
class SearrchORM(object):
    ...
    @classmethod
    def data_save(cls, msg: str) -> bool:
        with cls.sql_client().cursor() as cursor:
            try:
                data = {
                    'msg': msg,
    ...
    @classmethod
    def select_user(cls, token:str) -> bool:
        with cls.sql_client().cursor() as cursor:
            # data = {'token': token}
            token = token
            cursor.execute(
                # 'SELECT (`username`, `password`) from user where %(token)s',data
                # 'SELECT (`username`+`password`) as tt from user where %(token)s',data
                'SELECT `username`,`password` from user where token=token'
            )
